...
class DynamicColors: ObservableObject {
    ...
    init(fileName: String = "theme") {
        getFromJson(fileName: fileName)
    }
    
    func getFromJson(fileName: String) {
        let documentsURL = FileManager.default.urls(for: .documentDirectory, in: .userDomainMask).first!
        let themeURL = documentsURL.appendingPathComponent("\(fileName).json")
    ...
    func storeToJson() {
        ...
        if let jsonData = try? encoder.encode(colors) {
            ...
            if let documentsDirectory = FileManager.default.urls(for: .documentDirectory, in: .userDomainMask).first {
                let fileURL = documentsDirectory.appendingPathComponent("theme.json")
